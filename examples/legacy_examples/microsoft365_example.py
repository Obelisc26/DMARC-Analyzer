"""
Example: Analyze DMARC reports from Microsoft 365
"""
...
def main():
    """
    Example of analyzing DMARC reports from Microsoft 365 only
    """
    print("=" * 60)
    print("DMARC Analysis - Microsoft 365 Reports")
    print("=" * 60)
    ...
    # In this example, we assume all reports are from Microsoft 365
    if os.path.exists(reports_dir):
        xml_files = [f for f in os.listdir(reports_dir) if f.endswith('.xml')]
        
        if not xml_files:
            print("\nNo XML files found in reports directory.")
            print("Place your Microsoft 365 DMARC reports (XML format) in the 'reports' folder.")
            return
    ...
    output_file = 'microsoft365_dmarc_analysis.xlsx'
